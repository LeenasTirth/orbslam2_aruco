...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb/2), mpMap(pMap), hasMarker(F.hasMarker), markerIds(F.markerIds),
    canMarkerLoop(F.canMarkerLoop)
{
    mnId=nNextId++;
...
{   
    {
        unique_lock<mutex> lock(mMutexConnections);
        if(mnId==0 || hasMarker) // not delete the keyframe which has markers
            return;
        else if(mbNotErase)
        {
            mbToBeErased = true;
